...
// Reusable component: pass planId, renders all rooms and their seat grids
    ...
    useEffect(() => {
        const fetchPlan = async () => {
            ...
            try {
                ...
                const defaultRoom =
                    data.rooms?.[0]?.room_id ?? data.rooms?.[0]?.roomId ?? data.rooms?.[0]?.id ?? null;
        ...
        <div className="space-y-4">
            ...
            {activeRoom ? (
                <div className="rounded-3xl border border-white/10 p-4 bg-white/5">
                    <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${activeRoom.cols}, 1fr)` }}>
                        {Array.from({ length: activeRoom.rows }).map((_, rowIndex) =>
                            Array.from({ length: activeRoom.cols }).map((_, colIndex) => {
                                const row = rowIndex + 1;
                                const col = colIndex + 1;
                                const seat = activeAllocations.find((s) => s.seat_row === row && s.seat_col === col);
                                const key = `${row}-${col}`;
                                let seatClasses = 'rounded-2xl px-3 py-2 text-left border text-xs ';
                                if (seat) {
                                    seatClasses += 'border-brand-500/40 text-white ';
                                    seatClasses += seat.student_id === student_id ? 'bg-green-500/30' : 'bg-brand-500/10';
                                } else {
                                    seatClasses += 'bg-white/5 border-white/10 text-gray-500';
                                }

                                return (
                                    <div key={key} className={seatClasses}>
                                        {seat ? (
                                            <>
